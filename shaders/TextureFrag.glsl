...
vec4 pointLight()
{
  ...
  // Compute distance between surface and light position
    ...
    diffuse *= light.diffuse*lambertTerm;
    ambient+=material.ambient*light.ambient;
    halfV = normalize(halfVector);
    ndothv = max(dot(N, halfV), 0.5);
...
void main ()
{
 ...
 outColour = pointLight() /* abs(vec4(vertThing, 1.0f))*/;
}
